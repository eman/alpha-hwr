...
async def test_set_temperature_range_control_success(mock_client_simple):
    ...
    result = await mock_client_simple.control.set_temperature_range_control(35.0, 39.0, autoadapt=True)
...
async def test_set_temperature_range_control_no_autoadapt(mock_client_simple):
    ...
    result = await mock_client_simple.control.set_temperature_range_control(35.0, 39.0, autoadapt=False)
...
async def test_set_temperature_range_control_failure(mock_client_simple):
    ...
    mock_client_simple.transport.query = AsyncMock(side_effect=Exception("Transport error"))
    
    result = await mock_client_simple.control.set_temperature_range_control(35.0, 39.0)
