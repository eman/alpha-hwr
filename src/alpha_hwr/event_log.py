...
class EventLogEntry:
    ...
    def __init__(self, raw_data: bytes, subid: int):
        ...
        """
        if len(raw_data) != 16:
            raise ValueError(f"Event log entry must be 16 bytes, got {len(raw_data)}")
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "mode_char": chr(self.mode_byte) if 32 <= self.mode_byte <= 126 else None,
...
class CycleTimestampMap:
    ...
    def __init__(self, raw_data: bytes):
        ...
        """
        if len(raw_data) != 43:
            raise ValueError(f"Cycle timestamp map must be 43 bytes, got {len(raw_data)}")
        ...
        for i in range(10):
            offset = 3 + (i * 4)
            timestamp_raw = struct.unpack('>I', raw_data[offset:offset+4])[0]
